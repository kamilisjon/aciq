...
python -m examples.resnet_weights_analysis --model resnet18 --per-layer-rows 3 --per-layer-cols 7 --per-layer-quantile 99.9
python -m examples.resnet_weights_analysis --model resnet101 --per-layer-rows 11 --per-layer-cols 9 --per-layer-quantile 99.9  # For appendix
