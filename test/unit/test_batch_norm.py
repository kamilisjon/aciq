...
class TestFuseBnIntoConv(unittest.TestCase):
  ...
  def test_fused_weights_and_bias_match_onnx(self):
    ...
    for i, (conv_name, conv, bn_name, bn) in enumerate(pairs):
      ...
      np.testing.assert_allclose(onnx_unfused_weight, pre_weight, atol=1e-6, err_msg=f"Unfused weight mismatch at {conv_name}")
      np.testing.assert_allclose(manual_fused_weight, onnx_fused_weight, atol=1e-6, err_msg=f"Fused weight mismatch at {conv_name}")
      np.testing.assert_allclose(manual_fused_bias, onnx_fused_bias, atol=1e-6, err_msg=f"Fused bias mismatch at {conv_name}")
