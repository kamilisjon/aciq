...
\begin{itemize}
\item All math is per-output-channel, so per-tensor and per-output-channel weight quantization are both supported without changes.
\item Variance scaling is folded into the (already-quantized) weights and bias. After folding, the weights are no longer on the original symmetric quantization grid; this is acceptable for an analysis pipeline that stores dequantized FP32 representations of quantized weights, but on real hardware $s_{c_o}$ would more naturally be implemented as a per-output-channel rescale at the accumulator.
